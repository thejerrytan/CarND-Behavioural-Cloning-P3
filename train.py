    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size//6): # Because each loop generates x6 more images
            ...
            for batch_sample in batch_samples:
            	...
            	preprocess_data('./data/IMG/' + batch_sample[0], measurement, images, angles)
            	# Add left camera
            	measurement_left = measurement + CORRECTION_FACTOR
            	preprocess_data('./data/IMG/' + batch_sample[1], measurement_left, images, angles)
            	# Add right camera
            	measurement_right = measurement - CORRECTION_FACTOR
            	preprocess_data('./data/IMG/' + batch_sample[2], measurement_right, images, angles)
...
def preprocess_data(source_path, measurement, images, angles):
	image = cv2.imread('./data/IMG/' + source_path.split(os.sep)[-1])
